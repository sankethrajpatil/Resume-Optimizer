...
type ResumeJsonPayload = Pick<FormValues, 'skills' | 'projects' | 'sections'>
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.status(502).json({
        ...
        detail: errText || response.statusText
      })
      return
    }
    ...
    try {
      ...
      res
        .status(502)
        .json({ error: 'Failed to parse OpenAI JSON content', detail: String(e) })
      return
    }
